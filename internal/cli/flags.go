...
func ParseFlags() (Config, wordlist.Wordlist) {
	...
	pflag.IntVar(&cfg.Depth, "depth", 2, "Maximum recursive depth when --recursive is enabled; -1 means unlimited. Default: 2")
	...
	pflag.StringVarP(&cfg.RawTimeout, "timeout", "T", "5s", "Request timeout (e.g. 500ms, 2s; bare numbers are seconds). Default: 5s; port mode default: 500ms")
	pflag.StringVarP(&cfg.RawDelay, "delay", "d", "0", "Delay between requests (e.g. 200ms, 2s; bare numbers are milliseconds). Default: 0")
	pflag.StringSliceVarP(&cfg.UserAgent, "user-agent", "a", PreUserAgents, "User-Agents to rotate; repeat to add multiple values")
	// bools
	...
	pflag.BoolVarP(&cfg.ForceProxy, "force-proxy", "", false, "Skip unknown proxy scheme confirmation and continue direct")

	pflag.StringVarP(&cfg.Proxy, "proxy", "p", "", "HTTP, HTTPS, or SOCKS5 proxy (e.g. http://127.0.0.1:8080)")
	...
	pflag.StringSliceVarP(&cfg.Header, "header", "H", nil, "Add custom HTTP headers; repeat to add multiple values")
	...
	pflag.BoolVar(&cfg.JSON, "json", false, "Print results as JSON Lines")
